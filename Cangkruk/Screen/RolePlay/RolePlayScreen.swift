...
struct RolePlayScreen: View{
    ...
    private var bottomControls: some View {
        if viewModel.isPreparing {
            ...
            VStack(spacing: 12) {
                Text(viewModel.speechToText.isPlaying ? "TEKAN UNTUK BERHENTI" : "TEKAN UNTUK BERBICARA")
                    .font(.shakyComicBold(size: 14))
                    .foregroundStyle(Color("Secondary"))

                HoldToTalkButton(isRecording: viewModel.speechToText.isPlaying) {
                    Task {
                        if !sessionStarted {
                            sessionStarted = true
                            viewModel.startTimer()
                        }
                        await viewModel.speechToText.processMic()
                    }
                }
